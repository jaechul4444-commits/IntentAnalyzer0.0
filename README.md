# 🚀 Elastic Hybrid Search 기반 의도 분석 및 정비 데이터 검색 API (v1.6)
...
*   **🚀 LLM 자연어 답변 합성 제거 (응답 속도 극대화)**:
    *   기존 v1.5까지 수행되던 OpenAI API를 통한 최종 문장 합성(`generate_answer`) 단계를 전면 제거하여 응답 지연시간(Latency)을 기존 수 초에서 **밀리초(ms) 단위**로 비약적으로 단축시켰습니다.
    *   의도분석기(`IntentAnalyzer`)는 순수 검색 로우(Raw) 데이터와 통계만 즉시 반환하며, 마크다운 렌더링 및 스트리밍 역할은 챗봇 클라이언트 엔진(`chatbot_engine`)으로 이관되어 환각(Hallucination) 없는 100% 팩트 기반의 UI가 제공됩니다.
